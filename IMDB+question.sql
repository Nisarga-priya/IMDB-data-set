...
FROM MOVIE
INNER JOIN 
genre where movie.id=genre.movie_id
GROUP BY genre
...
FROM movie
INNER JOIN ratings ON movie.id=ratings.movie_id LIMIT 10;
...
-- Type your code below:
SELECT count(movie.id) as movies FROM movie
INNER JOIN ratings ON ratings.movie_id=movie.id
...
FROM movie INNER JOIN ratings ON ratings.movie_id=movie.id
